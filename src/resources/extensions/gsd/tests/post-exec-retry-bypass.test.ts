...
describe("Post-execution blocking failure retry bypass", () => {
  ...
  test("post-exec failure pause message includes failing check details", async () => {
    ...
    const messages = ctx.ui.notify.mock.calls.map((c: { arguments: unknown[] }) => String(c.arguments[0]));
    assert.ok(messages.some((m: string) => m.includes("Post-execution checks failed") && m.includes("[import] src/broken.ts:1")));

    const pauseArgs = pauseAutoMock.mock.calls[0]?.arguments?.[2] as { message?: string; category?: string } | undefined;
    assert.equal(pauseArgs?.category, "post-execution");
    assert.equal(pauseArgs?.message, "Post-execution checks failed at [import] src/broken.ts:1.");
  });
