...
| `GSD_WEB_AUTH_TOKEN` | generated by `gsd --web` unless auth is disabled | Local web bearer token required on `/api/*` requests. Ignored when `GSD_CLOUD_MODE=1`. |
...
| `GSD_WEB_PROJECT_CWD` | — | Default local project path for `gsd --web` when `?project=` is not specified. Not used in cloud mode. |
...
| `GSD_CLOUD_MODE` | (unset) | Set to `1` to run the Next.js web app in hosted cloud mode instead of local `gsd --web` mode. Requires `GATEWAY_INTERNAL_URL`, `GATEWAY_INTERNAL_TOKEN`, and `APP_BRIDGE_SECRET`. |
| `GATEWAY_INTERNAL_URL` | — | Required when `GSD_CLOUD_MODE=1`. Internal server-to-server base URL for the cloud gateway. |
| `GATEWAY_INTERNAL_TOKEN` | — | Required when `GSD_CLOUD_MODE=1`. Bearer secret used by the web app for gateway `/internal/rpc/token` and `/internal/fs` calls. |
| `APP_BRIDGE_SECRET` | — | Required when `GSD_CLOUD_MODE=1`. HMAC secret shared with the cloud SaaS for `/api/cloud/bootstrap` app bridge tokens and `gsd_cloud_session` cookies. |
