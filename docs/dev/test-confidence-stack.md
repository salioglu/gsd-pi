...
| Repo-wide coverage report | `npm run test:coverage:full` | `coverage-report` merges gate job lcov (PRs) | Report-only on PRs |
...
|-----------|-------------|------------|-------|
| `src/` + GSD extension | `node --test` on compiled `dist-test/` (c8 in CI) | `test:unit` → `test:coverage:unit` in CI | Primary app unit tests; compile via `test:compile` |
| Extension integration suites | `node --test` + `resolve-ts.mjs` (c8 in CI) | `test:integration` → `test:coverage:integration` in CI | ollama, async-jobs, browser-tools, search-the-web, bg-shell, slash-commands |
| `packages/*` | `node --test` (compiled to dist-test; c8 in CI) | `test:packages` → `test:coverage:packages` in CI | Every linkable package must have ≥1 test (`verify:workspace-coverage`) |
...
2. `test-unit` — unit tests under c8 (uploads `coverage/unit`)
3. `test-packages` — package tests under c8 (uploads `coverage/packages`)
4. `integration-tests` — integration suites under c8 (uploads `coverage/integration`)
5. `e2e` — smoke binary tests
6. `coverage-report` — merges gate lcov fragments only (report-only, runs after 2–4 succeed)
